...
class SimpleEnvRemoveBlock_pnp_mug:
    def __init__(self, 
                 xml_path,
        ...
        self.test_stats = {
            'total_tests': 0,
            'successful_tests': 0,  # 两个任务都成功
            'failed_tests': 0,
            'timeout_tests': 0,
            'task1_failed': 0,  # 任务1失败次数
            'task2_failed': 0,  # 任务2失败次数
            'task1_completed': 0,  # 任务1实际完成次数
            'task2_attempts': 0,  # 进入任务2阶段的轮次
            'task2_success': 0,  # 任务2实际成功次数
            'task1_success_times': [],  # 任务1成功时间
            'task2_success_times': [],  # 任务2成功时间
            'total_success_times': []   # 整体成功时间
        }
        self.current_test_round = 0
        self.max_test_rounds = 10  # 默认测试轮数
        self.task_start_time = None
        self.task1_start_time = None
        self.task2_start_time = None
        self.task_timeout = 120.0  # 每个任务2分钟超时
    ...
    def reset(self, seed = None):
        ...
        if self.is_testing:
            ...
            self.task1_start_time = time.time()
            self.task2_start_time = None
    ...
    def check_success(self):
        ...
        if self.obj_target == 'body_obj_block_red':
            ...
            if success and not self.task_states['remove_red_block']:
                ...
                # 记录任务1完成时间
                if self.is_testing and self.task1_start_time:
                    task1_time = time.time() - self.task1_start_time
                    self.test_stats['task1_success_times'].append(task1_time)
                    print(f"红块移除任务完成！用时: {task1_time:.1f}秒")
                else:
                    print("红块移除任务完成！")
            return success
            ...
            if np.linalg.norm(p_block[:2] - p_plate[:2]) < 0.1 and np.linalg.norm(p_block[2] - p_plate[2]) < 0.6 and self.env.get_qpos_joint('rh_r1') < 0.1:
                ...
                if p > 0.9:
                    if not self.task_states['place_blue_mug']:
                        ...
                        # 记录任务2完成时间
                        if self.is_testing and self.task2_start_time:
                            task2_time = time.time() - self.task2_start_time
                            self.test_stats['task2_success_times'].append(task2_time)
                            print(f"蓝杯放置任务完成！用时: {task2_time:.1f}秒")
                        else:
                            print("蓝杯放置任务完成！")
                    return True
        return False
    ...
    def switch_to_next_task(self):
        ...
        if self.current_task == 'remove_red_block' and self.task_states['remove_red_block']:
            ...
            # 开始任务2计时
            if self.is_testing:
                self.task2_start_time = time.time()
    ...
    def start_testing(self, max_rounds=10):
        ...
        print(f"开始双任务测试循环，总共将进行 {max_rounds} 轮测试")
        print("每轮测试包含: 1) 移除红块 2) 放置蓝杯")
        self.reset()
    ...
    def check_task_timeout(self):
        ...
        # 检查任务1超时
        if self.current_task == 'remove_red_block' and self.task1_start_time:
            elapsed_time = current_time - self.task1_start_time
            if elapsed_time > self.task_timeout:
                return True
        
        # 检查任务2超时
        elif self.current_task == 'place_blue_mug' and self.task2_start_time:
            elapsed_time = current_time - self.task2_start_time
            if elapsed_time > self.task_timeout:
                return True
        
        return False
    
    def handle_dual_task_completion(self, task1_success=False, task2_success=False, 
                                   task1_timeout=False, task2_timeout=False):
        """
        处理双任务完成情况
        args:
            task1_success: bool, 任务1是否成功
            task2_success: bool, 任务2是否成功
            task1_timeout: bool, 任务1是否超时
            task2_timeout: bool, 任务2是否超时
        returns:
        ...
        # 判断整体成功
        overall_success = task1_success and task2_success

        if task1_success:
            self.test_stats['task1_completed'] += 1

        if overall_success:
            self.test_stats['successful_tests'] += 1
            self.test_stats['task2_attempts'] += 1
            self.test_stats['task2_success'] += 1
            # 记录总体成功时间
            if self.task_start_time:
                total_time = time.time() - self.task_start_time
                self.test_stats['total_success_times'].append(total_time)
                print(f"第 {self.current_test_round + 1} 轮双任务测试成功！总用时: {total_time:.1f}秒")
        else:
            self.test_stats['failed_tests'] += 1
            # 分析失败原因
            if not task1_success:
                self.test_stats['task1_failed'] += 1
                if task1_timeout:
                    self.test_stats['timeout_tests'] += 1
                    print(f"第 {self.current_test_round + 1} 轮测试失败：任务1(移除红块)超时")
                else:
                    print(f"第 {self.current_test_round + 1} 轮测试失败：任务1(移除红块)未完成")
            elif not task2_success:
                self.test_stats['task2_failed'] += 1
                self.test_stats['task2_attempts'] += 1
                if task2_timeout:
                    self.test_stats['timeout_tests'] += 1
                    print(f"第 {self.current_test_round + 1} 轮测试失败：任务2(放置蓝杯)超时")
                else:
                    print(f"第 {self.current_test_round + 1} 轮测试失败：任务2(放置蓝杯)未完成")
        ...
        if self.current_test_round >= self.max_test_rounds:
            self.is_testing = False
            self.print_dual_task_results()
            # 自动关闭MuJoCo窗口
            self.close_viewer()
            return True
        
        # 重置环境进行下一轮测试
        print(f"准备进行第 {self.current_test_round + 1} 轮双任务测试...")
        self.reset()
        return False
    ...
    def print_dual_task_results(self):
        """
        打印双任务测试结果
        """
        ...
        task1_failed = self.test_stats['task1_failed']
        task2_failed = self.test_stats['task2_failed']
        task1_completed = self.test_stats['task1_completed']
        task2_attempts = self.test_stats['task2_attempts']
        task2_success = self.test_stats['task2_success']

        task1_times = self.test_stats['task1_success_times']
        task2_times = self.test_stats['task2_success_times']
        total_times = self.test_stats['total_success_times']
        ...
        print(f"双任务成功率: {success_rate:.2f}%")
        ...
        print(f"  任务1(移除红块)成功: {task1_completed} 轮 / 失败: {task1_failed} 轮 (成功率: {task1_success_rate:.2f}%)")
        print(f"  任务2(放置蓝杯)成功: {task2_success} 轮 / 失败: {task2_failed} 轮 (尝试: {task2_attempts} 轮, 全局成功率: {task2_success_rate_total:.2f}%, 尝试成功率: {task2_success_rate_attempts:.2f}%)")
        if task2_attempts < total:
            print(f"  任务2未尝试: {total - task2_attempts} 轮 (因任务1未完成)")
        
        # 时间统计
        if task1_times:
            avg_task1 = sum(task1_times) / len(task1_times)
            print(f"\n任务1时间统计:")
            print(f"  平均用时: {avg_task1:.1f}秒")
            print(f"  最短用时: {min(task1_times):.1f}秒")
            print(f"  最长用时: {max(task1_times):.1f}秒")
            print(f"  所有时间: {[f'{t:.1f}s' for t in task1_times]}")
        
        if task2_times:
            avg_task2 = sum(task2_times) / len(task2_times)
            print(f"\n任务2时间统计:")
            print(f"  平均用时: {avg_task2:.1f}秒")
            print(f"  最短用时: {min(task2_times):.1f}秒")
            print(f"  最长用时: {max(task2_times):.1f}秒")
            print(f"  所有时间: {[f'{t:.1f}s' for t in task2_times]}")
        
        if total_times:
            avg_total = sum(total_times) / len(total_times)
            print(f"\n双任务总体时间统计:")
            print(f"  平均总用时: {avg_total:.1f}秒")
            print(f"  最短总用时: {min(total_times):.1f}秒")
            print(f"  最长总用时: {max(total_times):.1f}秒")
            print(f"  所有总时间: {[f'{t:.1f}s' for t in total_times]}")
        ...
        return {
            ...
            'task1_failed': task1_failed,
            'task2_failed': task2_failed,
            'task1_completed': task1_completed,
            'task2_attempts': task2_attempts,
            'task2_success': task2_success,
            'success_rate': success_rate,
            'task1_success_times': task1_times,
            'task2_success_times': task2_times,
            'total_success_times': total_times,
            'task1_success_rate': task1_success_rate,
            'task2_success_rate_total': task2_success_rate_total,
            'task2_success_rate_attempts': task2_success_rate_attempts
        }
    
    def get_dual_task_progress(self):
        """
        获取当前双任务测试进度
        returns:
            dict: 测试进度信息
        ...
        if self.current_task == 'remove_red_block' and self.task1_start_time:
            task_elapsed = current_time - self.task1_start_time
        elif self.current_task == 'place_blue_mug' and self.task2_start_time:
            task_elapsed = current_time - self.task2_start_time
        
        if self.task_start_time:
            total_elapsed = current_time - self.task_start_time
        
        return {
            'current_round': self.current_test_round + 1,
            'total_rounds': self.max_test_rounds,
            'current_task': self.current_task,
            'task_elapsed_time': task_elapsed,
            'total_elapsed_time': total_elapsed,
            'timeout_limit': self.task_timeout,
            'task1_completed': self.task_states['remove_red_block'],
            'task2_completed': self.task_states['place_blue_mug'],
